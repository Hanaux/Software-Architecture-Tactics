		...
		while (!done) {
			...
			try {
				...
				case "6":
					...
					break;
				case "7":
					String studentId = giveStudent();
					String courseId = giveCourse();
					System.out.println(studentId+"is and"+courseId);
					eventBus.sendEvent(new Event(EventId.setStudent, studentId));
					Thread.sleep(2000);
					eventBus.sendEvent(new Event(EventId.setCourse, courseId));
					Thread.sleep(2000);
					eventBus.sendEvent(new Event(EventId.MakeReservation, studentId+" "+courseId));
					break;
				case "0":
